    ...
    const fetchLeakTrends = async () => {
      ...
      try {
        const response = await fetcher(
          `/api/interviews/company/${companyId}/analytics`
        );
    ...
    <div className="section-leaks-container">
      <div className="leak-analytics-header mb-4">
        <div>
          ...
          <p className="text-muted smallest m-0 text-uppercase ls-wide">
            Intelligence Report
          </p>
        </div>
      ...
      <Card className="analytics-hero-card border-0 mb-4">
        <Card.Body className="d-flex align-items-center justify-content-between p-4">
          <div className="hero-label">
            ...
            <p className="smallest text-muted m-0">
              Scale: 1.0 (Easy) - 5.0 (Hard)
            </p>
          </div>
          <div className="hero-value">
            <span className="display-5 fw-bold text-emerald">
              {analytics?.avgDifficulty || '0.0'}
            </span>
          </div>
        </Card.Body>
      </Card>
            ...
            {analytics.recentLeaks.map((leak, index) => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    className={`badge-pill-sm ${outcome === 'Offer' ? 'bg-offer' : 'bg-rejection'}`}
                  >
                    {outcome}
                  </span>
                </div>
