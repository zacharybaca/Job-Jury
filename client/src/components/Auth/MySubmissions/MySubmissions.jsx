...
const MySubmissions = () => {
  ...
  const handleDelete = async (id, name) => {
    if (
      window.confirm(
        `Are you sure you want to withdraw your submission for ${name}?`
      )
    ) {
  ...
  if (loading)
    return (
      <div className="submissions-loading">Retrieving your submissions...</div>
    );
  ...
  return (
    <div className="submissions-container">
      <header className="submissions-header">
        ...
        <p>
          Track the approval status of the employers you've brought to the Jury.
        </p>
      </header>
          ...
          {companies.map((company) => (
            <div key={company._id} className="submission-card">
              ...
              <div className="card-footer">
                {company.isApproved ? (
                  <Link to={`/companies/${company._id}`} className="view-link">
                    View Public Page
                  </Link>
                ) : (
